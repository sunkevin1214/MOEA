...
void EnvironmentalSelection(CPopulation *pnext, CPopulation *pcur, vector<CReferencePoint> rps, size_t PopSize)
{
	...
	// 判断下参考点是否有 potentialMember，没有的话那就肯定选不上来了啊
	double total = 0.0;
	for (int i = 0; i != rps.size(); i++)
	{
		if (rps[i].HasPotentialMember())
		{
			total += 1.0;
		}
	}
	double percentage = total / rps.size();
	...
	{
		cout << "To solutions assigned to each reference line" << endl;
	}
	// ---------- Step 17 / Algorithm 4 ----------
	while (next.size() < PopSize)
	{
		/*cout << next.size() << endl;*/
		size_t min_rp = FindNicheReferencePoint(rps);// 找到member size最小的参考点的索引

		int chosen = SelectClusterMember(rps[min_rp]); //找这个参考点，是否有在最有那个front中绑定的soulution，如果没有的话返回-1，如果有的话找最近的那个距离
		if (chosen < 0) // no potential member in Fl, disregard this reference point
		{
			rps.erase(rps.begin() + min_rp); // 这个参考点没油绑定最后那个front中的solution，所以给删除掉了
		}
		else
		{
			rps[min_rp].AddMember();  // 这个参考点绑定的有，所以就把member给加1.
			rps[min_rp].RemovePotentialMember(chosen);// 然后把这个solution给删除掉，估计是怕下次在选上吧
			next.push_back(cur[chosen]);
			//if (rps[min_rp].MemberSize() < 2)
			//{
			//	next.push_back(cur[chosen]);//保存这个参考点。 最后造成的可能的结果就是全部的点
			//}
			//else
			//{
			//	system("pause");
